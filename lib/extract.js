...
function extractMessages(blackList, callback) {
  ...
  function addToMsgs(msgArray) {
    if (msgArray) msgArray.forEach(function(m) {
      var hashKey = helper.hashKeys(m.msg);
      var key = hashKey ? md5(m.msg) : m.msg;
      if (m.loc) {
        if (!msgsLoc) msgsLoc = {};
        if (msgsLoc[key]) msgsLoc[key].push(m.loc);
        else msgsLoc[key] = [m.loc];
      }
      if (!hashKey) return;
      if (key in msgs) {
  ...
  if (msgFiles) msgFiles.forEach(function(msgFile) {
    ...
    fs.writeFileSync(msgFilePath,
      JSON.stringify(sortObject(jsonObj), null, 2) + '\n');
    if (msgsLoc) {
      if (jsonLocObj) {
        if (isMessagesJson) {
    ...
    if (jsonLocObj) {
      ...
      fs.writeFileSync(msgLocFilePath,
        JSON.stringify(sortObject(jsonLocObj), null, 2) + '\n');
    }
  });
