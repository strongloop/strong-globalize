...
function extractMessages(blackList, callback) {
  ...
  mkdirp.sync(helper.intlDir(helper.PSEUDO_LANG));
  var msgs = {};
  var msgsLoc = null;
  var msgCount = 0;
  var wordCount = 0;
  ...
  function addToMsgs(msgArray) {
    if (msgArray) msgArray.forEach(function(m) {
      if (helper.getTrailerAfterDot(m.msg) === 'txt') return;
      var key = md5(m.msg);
      if (key in msgs) {
        debug('*** Key %s exists and overwriting.', key, ':', m.msg);
        if (m.loc) msgsLoc[key].push(m.loc)
        return;
      }
  ...
  if (msgFiles) msgFiles.forEach(function(msgFile) {
    ...
    fs.writeFileSync(msgFilePath, JSON.stringify(jsonObj, null, 2) + '\n');
    if (msgsLoc) {
      var msgLocFilePath = path.join(pseudoLangDirPath, msgFile);
      var jsonLocObj = null;
      try {
        var jsonLocObj = JSON.parse(fs.readFileSync(msgLocFilePath));
      } catch (e) {
      }
      if (jsonLocObj) {
        var keys = Object.keys(jsonLocObj);
        keys.forEach(function(key) {
          if (helper.hashKeys(key)) delete jsonLocObj[key];
        });
        if (isMessagesJson) {
          for (var key in msgsLoc) jsonLocObj[key] = msgsLoc[key];
        }
      } else {
        jsonLocObj = msgsLoc;
      }
