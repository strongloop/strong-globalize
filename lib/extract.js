...
function extractMessages(blackList, deep, suppressOutput, suppressZz,
    callback) {
  var msgs = null;
  var msgsLoc = null;
  var msgCount = 0;
  ...
  _(files).keys().forEach(function(resolvedFileName) {
    processSourceFile(resolvedFileName, files, verboseMode, suppressZz);
  });
  ...
  if (msgFiles) msgFiles.forEach(function(msgFile) {
    ...
    if (suppressZz) {
      helper.nukeDirSync(pseudoLangDirPath);
    }
    if (!suppressZz && jsonLocObj) {
...
function processSourceFile(resolvedFileName, files, verboseMode, suppressZz) {
  var entry = files[resolvedFileName];
  if (entry.scanned) return entry;
  entry.scanned = true;
  var msgs = scanAst(entry.content, entry.fileName, verboseMode,
    files, suppressZz);
  if (msgs === null || msgs === undefined) {
    entry.skipped = true;
    return;
...
function scanAst(content, fileName, verboseMode, fileEntries, suppressZz) {
  var shebangExpr = /^\s*#\!.*?(\r\n|\r|\n)/m;
