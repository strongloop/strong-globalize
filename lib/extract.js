...
function extractMessages(blackList, deep, callback) {
  var msgs = null;
  var msgsLoc = null;
  var msgCount = 0;
  var wordCount = 0;
    ...
    msgArray.forEach(function(m) {
      ...
      if (m.loc) {
        ...
        if (msgsLoc.hasOwnProperty(key)) {
          Array.prototype.push.call(msgsLoc[key], m.loc);
        } else {
          msgsLoc[key] = new Array(m.loc);
        }
      }
  ...
  helper.enumerateFilesSync(helper.getRootDir(), blackList,
    'js', false, deep, function(content, fileName) {
      scannedJsCount++;
      var msgs = scanAst(content, fileName);
  ...
  if (!deep) helper.enumerateFilesSync(helper.getRootDir(), blackList,
    ['html', 'htm'], false, deep, function(content, fileName) {
      scannedHtmlCount++;
      var msgs = scanHtml(content, fileName);
      if (msgs === null || msgs === undefined) {
        skippedHtmlCount++;
        return;
      }
  ...
  if (!messagesJsonExists) {
    if (msgs) {
      ...
      fs.writeFileSync(msgFilePath, JSON.stringify(msgs, null, 2) + '\n');
    }
    if (msgsLoc) {
      mkdirp.sync(pseudoLangDirPath);
      var msgLocFilePath = path.join(pseudoLangDirPath, 'messages.json');
      fs.writeFileSync(msgsLoc, JSON.stringify(msgsLoc, null, 2) + '\n');
  ...
  console.log(
    '\n--- root: ' + global.STRONGLOOP_GLB.ROOT_DIR +
    '\n--- max depth: ' +
      (deep ? (global.STRONGLOOP_GLB.MAX_DEPTH === Number.MAX_SAFE_INTEGER ?
        'unlimited' : global.STRONGLOOP_GLB.MAX_DEPTH.toString()) : 'N/A') +
...
function invertLocObj(locObj) {
  ...
  _.forEach(locObj, function(v1, k) {
    v1.forEach(function(v2) {
      ...
      inv[fileName][lineNumber].push(k);
    });
  });
  return inv;
}
...
function scanAst(content, fileName) {
  ...
  function recordLiteralPosition(nd) {
    ...
    if (nd.type === 'Literal' && nd.value && typeof nd.value === 'string') {
      ...
      recordLiteralPosition(nd.left);
      recordLiteralPosition(nd.right);
    }
  }
