...
function scanAst(content, fileName, verboseMode) {
  ...
  function checkCallExp(ce, d) {
    if (!ce) return;
    if (ce.arguments) {
      ce.arguments.forEach(function(cenode) {
        if (cenode.type === 'CallExpression' && cenode.callee) {
          checkCallExp(cenode, d);
        }
      });
    }
    var argsParent = ce;
    ...
    if (argsParent.arguments) {
      argsParent.arguments.forEach(function(cenode) {
        if (cenode.type === 'CallExpression' && cenode.callee) {
          checkCallExp(cenode, d);
        }
      });
