...
function extractMessages(blackList, deep, suppressOutput, callback) {
  ...
  function extractFromJsonFile(literalArg) {
    console.log()
    var jsonPath = path.join(helper.getRootDir(), literalArg.msg);
    assert(jsonPath === path.resolve(jsonPath),
      '*** json file path must be a full path: %s', jsonPath);
    try {
      var contents = require(jsonPath);
    } catch (e) {
      console.log('*** json read failure: ', jsonPath);
      return;
    }
    try {
      ...
      console.log('*** key array parse failure: ', cleanStr);
      return;
    }
  ...
  function addToMsgs(msgArray) {
    ...
    msgArray.forEach(function(m, ix) {
      if (helper.getTrailerAfterDot(m.msg) === 'json') {
        var msgArrayFromJson = extractFromJsonFile(m);
        if (msgArrayFromJson)
