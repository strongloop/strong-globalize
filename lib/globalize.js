...
function formatMessage(path, variables, lang) {
  assert(path);
  if (helper.getTrailerAfterDot(path) === 'json') {
    return formatJson(path, variables, lang);
  }
  var message = path;
...
function formatJson(fullPath, variables, lang) {
  assert(fullPath === path.resolve(fullPath),
    '*** full path is required to format json file: ' + fullPath);
  try {
