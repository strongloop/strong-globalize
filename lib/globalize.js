...
function formatMessage(path, variables, lang) {
  ...
  var hashKeys = helper.hashKeys(path);

  if (hashKeys && cache[lang] && cache[lang][message])
    return cache[lang][message](variables);

  var cacheKey = message;

  if (hashKeys) path = md5(path);
  ...
  if (trailer === 'json' || trailer === 'yml' || trailer === 'yaml') {
    ...
    var res = formatJson(fullPath, JSON.parse(variables), lang);
    return res;
  }
  function formatMsgInline(lang, cacheLang) {
    var g = global.STRONGLOOP_GLB.bundles[lang];
    var formatter = g.messageFormatter(path);
    var message = formatter(variables || {});
    if (!(cacheLang in cache)) cache[cacheLang] = {};
    cache[cacheLang][cacheKey] = formatter;
    return message;
  }
  var resolved = false;
  try {
    message = formatMsgInline(lang, lang);
    resolved = true;
  } catch (e) {
    debug('*** %s for %s not localized *** %s', path, lang, e.stack);
  }

  if (!resolved && lang !== helper.ENGLISH) {
    debug('*** %s - falling back to English. ***', path);
    try {
      message = formatMsgInline(helper.ENGLISH, lang);
      resolved = true;
    } catch (e) {
      debug('*** %s not found for English *** %s', path, e.stack);
    }
  }

  if (!resolved) {
    if (!(lang in cache)) cache[lang] = {};
    cache[lang][cacheKey] = function(msg) {
      return function(variables) {
        return sanitizeMsg(msg, variables);
      };
    }(message);

    message = sanitizeMsg(message, variables);
    debug('*** %s not found for %s. Fall back to: "%s" ***',
      path, lang, message);
...
function ewrite() {
  return packMessage(arguments, function(msg) {
    logPersistent('error', msg);
    if (consoleEnabled()) process.stderr.write(msg.message);
  }, true);
}

function owrite() {
  return packMessage(arguments, function(msg) {
    logPersistent('info', msg);
    if (consoleEnabled()) process.stdout.write(msg.message);
  }, true);
}

function format() {
  return packMessage(arguments, null, false);
}
