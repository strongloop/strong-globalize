...
function loadGlobalize(lang, enumerateNodeModules) {
  ...
  if (!global.STRONGLOOP_GLB) {
    ...
    var versionG = helper.getPackageVersion(
      path.join(__dirname, '..', 'node_modules', 'globalize'));
    ...
    translate.loadMsgFromFile(helper.ENGLISH, null, enumerateNodeModules);
  }
  if (!(lang in global.STRONGLOOP_GLB.bundles)) {
    loadCldr(lang);
    global.STRONGLOOP_GLB.bundles[lang] = new Globalize(lang);
    global.STRONGLOOP_GLB.locale(lang);
    translate.loadMsgFromFile(lang, null, enumerateNodeModules);
    var dynamicPacks = global.STRONGLOOP_GLB.MSG_RES_LOADED;
    dynamicPacks.forEach(function(resTag) {
      translate.loadMsgFromFile(lang, resTag.rootDir, enumerateNodeModules);
    });
  }
...
function loadCldr(lang) {
  ...
  helper.enumerateFilesSync(cldrDir, null, ['gz'], false,
