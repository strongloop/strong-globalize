...
var BYPASS_EN_MSGES = true;

exports.bypassMsges = bypassMsges;
...
function bypassMsges(args, lang) {
  if (!BYPASS_EN_MSGES) return null;
  if (lang !== exports.ENGLISH) return null;
  var type = getTrailerAfterDot(args[0]);
  if (type === 'json' || type === 'yml' || type === 'yaml') return null;
  args[0] = args[0]
    .replace(/}}/g, '')
    .replace(/{{/g, '');
