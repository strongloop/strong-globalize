 ...
 */
function enumerateFilesSync(dir, blackList, targetFileType, callback) {
  ...
  files.forEach(function(item) {
    ...
    if (stats.isDirectory()) {
      ...
      enumerateFilesSync(child, blackList, targetFileType, callback);
    } else {
      var fileType = getTrailerAfterDot(item);
      if (targetFileType.indexOf(fileType) < 0) return;
      var content = fs.readFileSync(path.resolve(child), 'utf8');
      console.log('~~~ examining:', child);
      callback(content);
    };
  });
}
...
function enumerateMsgSyncPriv(rootDir, lang, checkNodeModules, callback) {
  ...
  msgFiles.forEach(function(msgFile) {
    ...
    if (jsonObj) callback(jsonObj, msgFile);
  });
  if (checkNodeModules) {
    ...
    moduleRootPaths.forEach(function(moduleRootPath) {
      ...
      enumerateMsgSyncPriv(moduleRootPath, lang, true, callback);
    });
  }
}
...
function isSupportedLanguage(lang) {
  return (lang === exports.ENGLISH || TARGET_LANGS.indexOf(lang) >= 0);
}
...
function setPersistentLogging(logFn, disableConsole) {
  assert(logFn);
  if (disableConsole) DISABLE_CONSOLE = true;
  try {
    logFn('info',
      'StrongGloablize persistent logging started at '
        + (new Date()).toString());
    LOG_FN = logFn;
  } catch (e) {
...
function logPersistent(message, level) {
  if (!LOG_FN) return;
  level = level || 'info';
