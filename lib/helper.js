...
function alreadyScanned(fileName) {
  var realFileName = fs.realpathSync(fileName);
...
function enumerateMsgSyncPriv(currentPath, rootDir, lang, checkNodeModules,
  ...
  msgFiles.forEach(function(msgFile) {
    ...
    if (cloneEnglishTxt && (lang === exports.ENGLISH)) {
      ...
      var enDirPath = path.join(rootDir, 'intl', exports.ENGLISH);
      var targetTxtFilePath = path.join(enDirPath,
        filePathHash + '_' + msgFile);
...
function replaceJson(keys, data, newData) {
  return scanJsonPriv(keys, data, newData, false);
}

function scanJsonPriv(keys, data, newData, returnErrors) {
  var ret = [];
  if (!keys || !Array.isArray(keys)) return [];
  if (!data || typeof data !== 'object') return [];
  if (newData) assert(keys.length === newData.length);
  keys.forEach(function(k, kix) {
    ...
    try {
      for (var ix = 0; ix < k.length; ix++) {
        if (ix === 0) d = this;
        if (typeof d === 'string') {
          var err = '*** unexpected string value ' + JSON.stringify(k);
      ...
      if (typeof d === 'string') {
        if (newData) prevObj[prevKey] = newData[kix];
        else ret.push(d);
      } else {
        var err = '*** not a string value ' + JSON.stringify(k);
        if (returnErrors) ret.push(err);
        else console.log(err);
      }
    } catch (e) {
      var err = '*** ' + e.toString() + ' ' + JSON.stringify(k);
      if (returnErrors) ret.push(err);
      else console.log(err);
    }
  }.bind(data));
  return (newData ? data : ret);
}
...
function getSupportedLanguages() {
  ...
  enumerateFilesSync(cldrDir, null, ['gz'], false, false,
    ...
    try {
      ...
      throw new Error('*** CLDR unzip error on ' + process.platform);
    }
