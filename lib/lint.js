...
var GPB_MAX_VALUE_LENGTH = 81920000;
...
function lintMessageFiles(enOnly, callback) {
  ...
  function extractTrait(lang) {
    ...
    helper.enumerateMsgSync(rootDir, lang, false, function(jsonObj) {
      trait.nFiles++;
  ...
  function checkTrait(trait) {
    ...
    if (Object.keys(trait.msgs).length >= GPB_MAX_NUMBER_OF_KEYS) {
      console.error('***', trait.bundleName,
  ...
  if (!helper.initIntlDirs()) {
    console.error('Intl directory is not set up.');
    return;
  }
