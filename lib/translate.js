...
// GPB service limits
var GPB_MAX_NUMBER_OF_KEYS = 500; // per messages.json
var GPB_MAX_KEY_LENGTH = 256; // max number of characters
var GPB_MAX_VALUE_LENGTH = 8192; // max number of chars in En content per msg
var MY_TRANSLATION_UNIT_DEFAULT = 500; // No. of keys to send to GPB per bundle
var MY_TRANSLATION_UNIT = MY_TRANSLATION_UNIT_DEFAULT;
assert(MY_TRANSLATION_UNIT <= GPB_MAX_NUMBER_OF_KEYS);

function setTranslationUnit(unit) {
  if (unit && (unit <= 0 || unit > MY_TRANSLATION_UNIT_DEFAULT))
    unit = MY_TRANSLATION_UNIT_DEFAULT;
  MY_TRANSLATION_UNIT = unit;
}
...
function storeMsg() {
  fs.writeFileSync(path.join(INTL_DIR, 'MSG.json'),
    JSON.stringify(msgStore, null, 2) + '\n');
}
...
function translateResource(callback) {
  ...
  gpClient.supportedTranslations({}, function(err, supportedLangs) {
    ...
    mktmpdir(function(err, tempDir, done) {
      translateResourcePriv(gpClient, langs, tempDir, function(err) {
        done();
        return callback(err);
      });
    }, function(err, dir) {
  ...
  langDirs.forEach(function(lang) {
    ...
    bases.forEach(function(base) {
      fs.writeFileSync(path.join(intlDir, lang, base) + '.json',
        JSON.stringify(jsonData[base], null, 2));
    });
  });
}
