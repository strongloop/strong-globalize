  ...
  helper.enumerateMsgSync(rootDir, lang, enumerateNodeModules,
    function(jsonObj, filePath) {
      ...
      if (results && results.length === 3) { // deep-extracted txt file ?
        filePathHash = results[1];
        fileName = results[2] + '.txt';
        appName = null; // unknown
        appVersion = null; // unknown
      } else {
        filePathHash = md5(filePath);
        fileName = fileName;
        appName = helper.getPackageName(rootDir);
        appVersion = helper.getPackageVersion(rootDir);
      }
      if (helper.resTagExists(
          filePathHash, fileName, appName, appVersion, lang, tagType)) {
        debug('*** loadMsgFromFile res tag exists:', lang, fileName);
        return;
      }
      ...
      helper.registerResTag(filePathHash, fileName, appName,
