  ...
  mktmpdir(
    function(err, destDir, done) {
      ...
      function checkErrMsg(outMsg, errMsg, key, targets, t) {
        ...
        if (!DEBUG) {
          ...
          errMsg.forEach(function(err, ix) {
            ...
            if (temporaryFailure) return;
            t.match(err, targets[key].err[ix], key + ': err msg matches.');
          });
          if (temporaryFailure) return;
          t.equal(
            outMsg.length,
            targets[key].out.length,
            'out msg count matched: ' + key
          );
          t.equal(
            errMsg.length,
            targets[key].err.length,
      ...
      function initRootDir(dir) {
        ...
        // the heavy weight STRONGLOOP_GLB initialization.
      ...
      if (!noFixtures) {
        t.comment(
          '--- copying %d: %s to %s',
          copyDirs.length,
          JSON.stringify(copyDirs, null, 2),
          destDir
        );
