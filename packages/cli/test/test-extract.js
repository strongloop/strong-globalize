...
var g = require('strong-globalize/lib/globalize');
var helper = require('strong-globalize/lib/helper');
var extract = require('../lib/extract');
var test = require('tap').test;

var testFileName = 'test-extract.js';

var content_singleton_head =
  '// line 1\n//line 2\n//line 3\n' + 'var g = require("strong-globalize");\n';
var content_multiple_head =
  '// line 1\n//line 2\n//line 3\n' +
  'var g = require("strong-globalize")();\n';
var content_singleton_body =
  'function test() {\n' +
...
function subTest(mode, t) {
  ...
  t.equal(
    JSON.stringify(extractedLocs),
    JSON.stringify(targetLocs),
    mode + ': all locs extracted from JS.'
  );
  var enBundlePre = SG.STRONGLOOP_GLB.bundles[helper.ENGLISH];
  ...
  t.comment(resultMsg);
  t.equal(resultMsg, targetMsg, "filled t in array '" + targetMsg + "'");
  resultMsg = g.formatMessage(targetMsgs[baseIndex + 2], {0: 'zero', 1: 'one'});
  t.comment(resultMsg);
  t.equal(resultMsg, targetMsg, "filled t in object 0, 1 '" + targetMsg + "'");
  ...
  resultMsg = g.formatMessage(targetMsgs[baseIndex + 2], ['zero', 'one']);
  t.comment(resultMsg);
  t.equal(resultMsg, targetMsg, "filled format in array '" + targetMsg + "'");
  resultMsg = g.formatMessage(targetMsgs[baseIndex + 2], {0: 'zero', 1: 'one'});
  t.comment(resultMsg);
  t.equal(
    resultMsg,
...
test('pseudo loc extraction', function(t) {
  ...
  g.setDefaultLanguage();
  var content =
    "process.stdout.write('ps_arg1' + 'ps_arg2')\n" + "console.log('cs_arg')";
  var extractedMsgs = [];
  var extractedLocs = [];
  var targetMsgs = [
