...
test('SetDefaultLanguage', function(t) {
  ...
  var systemLocale = SG.STRONGLOOP_GLB.locale();
  t.assert(
    systemLocale &&
      systemLocale.attributes &&
      systemLocale.attributes.bundle &&
...
test('formatNumber', function(t) {
  ...
  t.equal(message, targetMsg, 'New instantiation to default works.');

  var gE = new SG({language: 'en'});
  message = gE.n(value);
  t.comment(message);
  targetMsg = '123,456.789';
  t.equal(message, targetMsg, 'English instalce works.');

  g.setLanguage('de');
  message = g.n(value);
  t.comment(message);
  targetMsg = '123.456,789';
  t.equal(message, targetMsg, 'Setting number format to German works.');
...
test('formatDate', function(t) {
  ...
  t.assert(message === targetMsg, 'English instalce works.');

  g.setLanguage('de');
  message = g.d(date);
  t.comment(message);
  targetMsg = '29.12.2015, 19:40:50';
  t.assert(message === targetMsg, 'Setting date format to German works.');
...
test('formatCurrency', function(t) {
  ...
  t.equal(message, targetMsg, 'English instance works.');

  g.setLanguage('de');
  message = g.c(value, symbol);
  t.comment(message);
  targetMsg = '123.456,79 US-Dollar';
  t.equal(message, targetMsg, 'Setting currency format to German works.');
...
test('throw Error', function(t) {
  ...
  try {
    ...
    t.equal(e.toString(), targetMsg, 'Error is correctly thrown and caught.');
  }
  t.end();
});
