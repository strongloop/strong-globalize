...
test('setDefaultLanguage', function(t) {
  ...
  t.assert(
    supportedLangs.indexOf(helper.ENGLISH) >= 0,
    helper.ENGLISH + ' should be supported.'
  );
  t.assert(supportedLangs.indexOf('ja') >= 0, 'ja should be supported.');
  g.setDefaultLanguage('ja');
  systemLocale = g.STRONGLOOP_GLB.locale();
  t.assert(
    systemLocale &&
      systemLocale.attributes &&
      systemLocale.attributes.bundle &&
      systemLocale.attributes.bundle === 'ja',
    'System language is set to ja.'
  );
  g.setDefaultLanguage();
  var systemLocale = g.STRONGLOOP_GLB.locale();
  t.assert(
    systemLocale &&
      systemLocale.attributes &&
      systemLocale.attributes.bundle &&
...
test('formatMessage', function(t) {
  ...
  message = g.formatMessage(key, params);
  t.comment(message);
  targetMsg = 'Error:' + params.url + 'あるいは' + params.port + '無効です。';
  ...
  t.comment(message);
  targetMsg =
    'Error: ' + params.url + '이나 ' + params.port + '은 효력이 없다.';
  ...
  t.comment(message);
  targetMsg =
    'Error: ' + params.url + ' oder ' + params.port + ' sind ungültig.';
  t.equal(message, targetMsg, 'German message formatting works.');

  g.setDefaultLanguage('es');
  message = g.formatMessage(key, params);
  t.comment(message);
  targetMsg = 'Error: ' + params.url + ' o ' + params.port + ' no es válido.';
  ...
  t.comment(message);
  targetMsg =
    'Error: ' + params.url + ' ou ' + params.port + " n'est pas valide.";
  ...
  t.comment(message);
  targetMsg =
    'Error: ' + params.url + ' o ' + params.port + ' non sono validi.';
  t.equal(message, targetMsg, 'Italian message formatting works.');

  g.setDefaultLanguage('pt');
  message = g.formatMessage(key, params);
  t.comment(message);
  targetMsg = 'Error: ' + params.url + ' ou ' + params.port + ' é inválido.';
...
test('formatNumber', function(t) {
  ...
  t.comment(message);
  targetMsg = '123,456.789';
  t.equal(message, targetMsg, 'Simplified Chinese number formatting works.');

  g.setDefaultLanguage('zh-Hant');
  message = g.n(value);
  t.comment(message);
  targetMsg = '123,456.789';
  t.equal(message, targetMsg, 'Traditional Chinese number formatting works.');
...
test('formatDate', function(t) {
  ...
  t.assert(message === targetMsg, 'Simplified Chinese date formatting works.');

  g.setDefaultLanguage('zh-Hant');
  message = g.d(date);
  t.comment(message);
  targetMsg = '2015年12月29日 下午7:40:50';
  t.assert(message === targetMsg, 'Traditional Chinese date formatting works.');
...
test('formatCurency', function(t) {
  ...
  t.equal(message, targetMsg, 'Simplified Chinese currency formatting works.');

  g.setDefaultLanguage('zh-Hant');
  message = g.c(value, symbol);
  t.comment(message);
  targetMsg = '123,456.79 美元';
  t.equal(message, targetMsg, 'Traditional Chinese currency formatting works.');
...
test('throw Error', function(t) {
  ...
  try {
    ...
    var targetMsg =
      'Error: Error:' + params.url + 'あるいは' + params.port + '無効です。';
    t.comment(e.toString());
    t.match(e.toString(), targetMsg, 'Error is correctly thrown and caught.');
  }
  t.end();
});
